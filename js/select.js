...
class CustomSelect {
  ...
  _reset() {
    ...
    this._elToggle.textContent = 'Level 16 (8 BNB)';
    this._elToggle.value = 'level16';
    this._elToggle.dataset.index = 14;
    this._elRoot.dispatchEvent(new CustomEvent('select.change'));
...
CustomSelect.template = params => {
  ...
  let items = [];
  let selectedIndex = 14;
  let selectedValue = 'level16';
  let selectedContent = 'Level 16 (8 BNB)';
  ...
  return `<button type="button" class="select__toggle" name="${name}" value="${selectedValue}" data-select="toggle" data-index="${selectedIndex}">${selectedContent}</button>
    <div class="select__dropdown">
        <div class="select-wrapper custom-scroll-select">
